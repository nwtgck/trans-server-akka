...
class Core(db: Database, fileDbPath: String){
  ...
  def processGetParamsRoute(f: GetParams => Route): Route = {
    ...
    parameter("duration".?, "get-times".?, "length".?, "deletable".?, "key".?) { (durationStrOpt: Option[String], nGetLimitStrOpt: Option[String], idLengthStrOpt: Option[String], isDeletableStrOpt: Option[String], deleteKeyOpt: Option[String]) =>
